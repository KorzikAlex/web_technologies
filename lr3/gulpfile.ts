...
const __filename: string = fileURLToPath(import.meta.url); // Получение имени файла
const __dirname: string = path.dirname(__filename); // Получение директории файла

// Инициализация SASS процессора
...
export function scripts() {
    return gulp.src(
        ['src/client/**/*.ts']
    ).pipe(tsProject()).js.pipe(gulp.dest(
        'public/gulp-build/js'
    ));
}
...
export function assets() {
    return gulp.src(
        ...
        gulp.dest('public/gulp-build/assets')
    );
}
...
export function fonts() {
    return gulp.src(
        ...
        gulp.dest('public/gulp-build/fonts')
    );
}
