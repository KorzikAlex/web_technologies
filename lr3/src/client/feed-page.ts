...
let feedPosts: PostWithAuthor[] = [];
let addPostModal: Modal | null = null;
let editPostModal: Modal | null = null;
let deletePostModal: Modal | null = null;
let postIdToDelete: number | null = null;
let postIdToEdit: number | null = null;
...
function attachPostEventListeners(): void {
    // Редактирование
    document.querySelectorAll('.edit-post').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.preventDefault();
            const postId = parseInt((e.currentTarget as HTMLElement).dataset.id || '0', 10);
...
async function handleAddPost(): Promise<void> {
    ...
    const image = (document.getElementById('postImage') as HTMLInputElement).value.trim();
    ...
    try {
        ...
        if (!response.ok) throw new Error('Ошибка при создании новости');
...
function openEditModal(postId: number): void {
    ...
    postIdToEdit = postId;
    (document.getElementById('editPostId') as HTMLInputElement).value = postId.toString();
    (document.getElementById('editPostContent') as HTMLTextAreaElement).value = post.content;
    (document.getElementById('editPostImage') as HTMLInputElement).value = post.image || '';
...
async function handleEditPost(): Promise<void> {
    ...
    const content = (document.getElementById('editPostContent') as HTMLTextAreaElement).value.trim();
    const image = (document.getElementById('editPostImage') as HTMLInputElement).value.trim();
    ...
    try {
        ...
        if (!response.ok) throw new Error('Ошибка при редактировании новости');
